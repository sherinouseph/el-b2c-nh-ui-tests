...
<body  bgcolor="#F8F8F8 ">
  <table>
       <tr>
           <td><a href="http://gblcm-l0321:8080/urlmonitor/urlmonitor-tc2.jsp"> <img src="logo.png?"> </a>                 </td>
  ...
  <%
	...
	try{
		...
		if(session.getAttribute("session")=="TRUE"){
			//out.println("Session Attribute is true :"); //+session.getAttribute("session"));  
		}else{
		    out.println("User Need to login ...!"); //Session Attribute is false/null:+session.getAttribute("session")); 
			response.sendRedirect("index.jsp");
		}
		Class.forName("com.mysql.jdbc.Driver").newInstance(); 
		conn  = DriverManager.getConnection( dbUrl, "javaRead","pass");
